# LegaCity
Open Source Launcher for Minecraft Legacy Console Edition

Built with VS 2026 using .NET and Avalonia UI for SmartCMD's MinecraftConsoles repo
Compatible with Windows 7 SP1 and up (don't know about mac or linux)

This was actually very simple to make, just an app with a fancy ui that can download from SmartCMD's nightly build, extract it and run it with username and fullscreen flags.

Installer made with InstallForge.

Goals:
- Mod support
- Our own modding framework
- Dedicated server launcher
